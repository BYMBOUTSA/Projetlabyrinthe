...
/*
Cell &Labyrinth::operator()(const Coordinate &c)
{
    return graph.at(c);
}

const Cell &Labyrinth::operator()(const Coordinate &c) const
{
    return graph.at(c);
}
*/
...
{
	const int valid_id = (*this)({0,0}).get_id();
...
void Labyrinth::fusion()
{
	while (!is_fusion_finished())
	{
		...
		// DEBUG std::cout << "Cellule: (" << coord1.first << "," << coord1.second << ") voisin de " << 
		// DEBUG	"(" << coord2.first << "," << coord2.second << ")\n";
		
		const int id_to_spread = (*this)(coord1).get_id();
		// DEBUG std::cout << "id to spread: " << id_to_spread << '\n';
		
		if ( id_to_spread != (*this)(coord2).get_id())
		{
			...
			while (!coord_to_propagate.empty())
			{
				...
				Cell &current_cell = (*this)(current_coord);
				
				// DEBUG std::cout << "début de la boucle while avec la cellule:" << current_cell << '\n';
				...
				(*this)(current_coord).set_id(id_to_spread);
